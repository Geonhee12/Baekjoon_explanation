...
void quicksort_x(int start, int end, struct coodination * sort_x) // x값을 토대로 퀵 정렬
{
    ...
    if(left >= right)
        return;
    int pivot = sort_x[(start+end) / 2].y;

    
    while(left <= right)
    {
        while(sort_x[left].y < pivot)
            left++;
        while(sort_x[right].y > pivot)
            right--;
        if(left <= right)
        {
            swap = sort_x[left];
...
void y_sort(int start, int end, struct coodination * sort_y) // x값을 토대로  y값 퀵 정렬
{
    ...
    if(left >= right)
        return;
    int pivot = sort_y[(start+end) / 2].x;

    
    while(left <= right)
    {
        while(sort_y[left].x < pivot)
            left++;
        while(sort_y[right].x > pivot)
            right--;
        if(left <= right)
        {
            swap.x = sort_y[left].x;
            sort_y[left].x = sort_y[right].x;
            sort_y[right].x = swap.x;
            left++;
            right--;
        }
    }
